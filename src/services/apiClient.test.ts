...
describe('apiClient', () => {
  ...
  // inaczej literówka w haśle powoduje przekierowanie zamiast komunikatu
    ...
    // przechwytujemy błąd, żeby sprawdzić i typ, i status w jednym wywołaniu
    const error = (await apiFetch("/auth/login", { method: "POST" }).catch(
      (e) => e,
    )) as ApiRequestError;

    expect(error).toBeInstanceOf(ApiRequestError);
    // status musi dotrzeć do formularza, inaczej LoginPage nie pokaże właściwego komunikatu (C2)
    expect(error.status).toBe(401);

    // REGRESJA: 401 bez tokenu to błąd logowania, nie wygaśnięcie sesji
