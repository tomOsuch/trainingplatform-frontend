...
function WorkoutLogForm({
  categories, log, initial, planTitle, onClose, onSaved,
}: WorkoutLogFormProps) {
  const editMode = Boolean(log);
  const today = toISODate(new Date());

  const [performedDate, setPerformedDate] = useState(
    log?.performedDate ?? initial?.performedDate ?? today
  );
  const [categoryId, setCategoryId] = useState(
    String(log?.categoryId ?? initial?.categoryId ?? "")
  );
  const [durationMin, setDurationMin] = useState(
    String(log?.durationMin ?? initial?.durationMin ?? "")
  );
  ...
  return (
    <Modal title={editMode ? "Edytuj wpis" : "Nowy wpis"} onClose={onClose}>
      <form className={styles.form} onSubmit={handleSubmit} noValidate>
        {planTitle && (
          <div className={styles.planBadge}>Powiązany z planem: {planTitle}</div>
        )}
        ...
        <label className={styles.field}>
          ...
          {errors.performedDate && (
            <span className={styles.fieldError}>{errors.performedDate}</span>
          )}
        </label>
